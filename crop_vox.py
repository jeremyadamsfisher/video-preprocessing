...
def save_bbox_list(video_path, bbox_list):
    f = open(
        os.path.join(args.bbox_folder, os.path.basename(video_path)[:-4] + ".txt"), "w"
    )
...
def estimate_bbox(person_id, video_id, video_path, fa, args):
    ...
    try:
        for i, frame in enumerate(reader):
            ...
            frame = resize(
                frame, (REF_FRAME_SIZE, int(frame.shape[1] / mult)), preserve_range=True
            )
...
def store(
    ...
    out, final_bbox = crop_bbox_from_frames(
        frame_list,
        tube_bbox,
...
def crop_video(person_id, video_id, video_path, args):
    ...
    try:
        for i, frame in enumerate(reader):
            ...
            if (
                bb_intersection_over_union(initial_bbox, bbox) < args.iou_with_initial
                or len(frame_list) >= args.max_frames
            ):
                chunks_data += store(
                    frame_list,
                    tube_bbox,
...
def split_in_utterance(person_id, video_id, args):
    ...
    utterances = [
        pd.read_csv(os.path.join(utterance_folder, f), sep="\t", skiprows=6)
    ...
    for i, utterance in enumerate(utterances):
        first_frame, last_frame = (
            utterance["FRAME "].iloc[0],
            utterance["FRAME "].iloc[-1],
        )
            ...
            if args.crop:
                ...
                for chunk in glob.glob(path):
                    if not os.path.exists(
                        os.path.join(
                            args.bbox_folder, os.path.basename(chunk)[:-4] + ".txt"
                        )
                    ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--min_frames", default=64, type=int, help="Mimimal number of frames"
    )
    parser.add_argument(
        "--max_frames", default=1024, type=int, help="Maximal number of frames"
    )
    parser.add_argument(
        "--min_size", default=256, type=int, help="Minimal allowed size"
    )
    parser.add_argument("--format", default=".png", help="Store format (.png, .mp4)")

    parser.add_argument(
        "--annotations_folder", default="txt", help="Path to utterance annotations"
    )

    parser.add_argument(
        "--video_folder", default="videos", help="Path to intermediate videos"
    )
    parser.add_argument(
        "--chunk_folder", default="chunks", help="Path to folder with video chunks"
    )
    parser.add_argument(
        "--bbox_folder", default="bbox", help="Path to folder with bboxes"
    )
    parser.add_argument(
        "--out_folder", default="vox-png", help="Folder for processed dataset"
    )
    parser.add_argument(
        "--chunks_metadata", default="vox-metadata.csv", help="File with metadata"
    )

    parser.add_argument(
        "--youtube", default="./youtube-dl", help="Command for launching youtube-dl"
    )
    parser.add_argument(
        "--workers", default=1, type=int, help="Number of parallel workers"
    )
    parser.add_argument(
        "--device_ids", default="0", help="Names of the devices comma separated."
    )
    ...
    parser.add_argument(
        "--no-split-in-utterance",
        dest="split_in_utterance",
        action="store_false",
    ...
    parser.add_argument(
        "--no-crop", dest="crop", action="store_false", help="Do not crop the videos"
    )

    parser.add_argument(
        "--remove-intermediate-results",
        dest="remove_intermediate_results",
        action="store_true",
