...
def crop_bbox_from_frames(
    ...
    if (
        max(
            left_oob / float(width),
            right_oob / float(width),
            top_oob / float(height),
            bot_oob / float(height),
        )
        > 0
    ):
        return [None, None]
    ...
    if image_shape is not None:
        out = [
            img_as_ubyte(resize(frame, image_shape, anti_aliasing=True))
...
def scheduler(data_list, fn, args):
    ...
    for chunks_data in tqdm(
        pool.imap_unordered(fn, zip(data_list, cycle(device_ids), args_list))
    ):
