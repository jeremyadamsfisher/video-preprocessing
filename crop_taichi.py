...
def process_video(video_path, detector, args):
    ...
    try:
        for i, frame in enumerate(video):
            ...
            height_criterion = (
                (bboxes[:, 3] - bboxes[:, 1]) > args.mimial_person_size * frame.shape[1]
            ).numpy()
            ...
            if previous_frame is None:
                ...
                previous_intensity = np.median(
                    frame.reshape((-1, frame.shape[-1])), axis=0
                )
            ...
            intensity_criterion = (
                np.max(np.abs(previous_intensity - current_intensity))
                > args.intensity_change_threshold
            )
            ...
            for bbox in bboxes_valid:
                ...
                if not intersect:
                    trajectories.append(
                        [compute_aspect_preserved_bbox(bbox, args.increase), i, [frame]]
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device_ids", default="0,1", type=str, help="Device to run video on"
    )
    ...
    parser.add_argument(
        "--increase", default=0.05, type=float, help="Increase bbox by this amount"
    )
    parser.add_argument(
        "--min_frames", default=128, type=int, help="Mimimal number of frames"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out_folder", default="taichi-256", help="Folder with output videos"
    )
    parser.add_argument(
        "--annotation_folder",
        default="taichi-annotations",
        help="Folder for annotations",
    )
    ...
    parser.add_argument(
        ...
        default=0.9,
        type=float,
        help="Maskrcnn confidence for bbox",
    )
    parser.add_argument(
        "--kp_confidence_th",
        default=2,
        type=float,
        help="Maskrcnn confidence for keypoint",
    )
    parser.add_argument(
        "--maskrcnn_config",
    ...
    parser.add_argument(
        "--minimal_video_size", default=300, type=int, help="Minimal size of the video"
    )
    ...
    parser.add_argument(
        "--max_crops", type=int, default=1000, help="Maximal number of crops per video."
    )
    parser.add_argument(
        "--chunks_metadata",
        default="taichi-metadata.csv",
        help="File to store metadata for taichi.",
    )
